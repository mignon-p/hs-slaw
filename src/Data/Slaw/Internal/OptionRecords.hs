...
data WriteYamlOptions = WriteYamlOptions
  { -- | Set it to false if you don't need full fidelity, and would
    -- rather not have all your numbers tagged.
    --
    -- [key]: tag_numbers
    --
    -- [type]: boolean
    ...
    -- | Set it to false if you don't want to emit the @%YAML@ and
    -- @%TAG@ directives.  This makes the tag name more verbose,
    -- and the YAML version unspecified.
    --
    -- [key]: directives
    --
    -- [type]: boolean
  ...
  , wyoDirectives       :: Maybe Bool
    -- | If true, encodes slaw maps as Yaml's @!!omap@.  If false,
    -- encodes slaw maps as Yaml's @!!map@ (which means the
    -- ordering might be lost).
    --
    -- [key]: ordered_maps
    --
    -- [type]: boolean
  ...
  , wyoOrderedMaps      :: Maybe Bool
    -- | If true, starts the file with a comment which includes the
    -- g-speak and libYaml version numbers, and a mode comment to
    -- prevent emacs from going into idlwave-mode for files with a
    -- .pro extension.
    --
    -- [key]: comment
    --
...
data PoolCreateOptions = PoolCreateOptions
  ...
  , pcoTocCapacity  :: Maybe Integer
    -- | If true, then the pool stops allowing deposits once it is
    -- full.  If false, the default, it will wrap around and delete
    -- the oldest proteins as newer ones are deposited.
    --
    -- [key]: stop-when-full
    --
    -- [type]: boolean
  ...
  , pcoStopWhenFull :: Maybe Bool
    -- | Makes the pool read-only until “frozen” is set to
    -- false again.
    --
    -- [key]: frozen
    --
  ...
  , pcoSync         :: Maybe Bool
    ...
    -- When false, Plasma uses System V semaphores.  The default is
    -- to use semaphores on Linux, and @flock()@ on macOS.  Windows
    -- always uses Windows mutexes, and is not affected by this
    -- option.
    --
    -- [key]: flock
    --
  ...
  , pcoChecksum     :: Maybe Bool
    ...
    -- octal number.  We recommend only using “7” or “0” in
    -- each position, since write permission is needed to read from
